...
const TrackingView = () => {
  useEffect(() => {
    ...
    script.innerHTML = `
        ...
        const endPoint = getPoint(endX, endY);
        const centerX = (startX + endX) / 2;
        const centerY = (startY + endY) / 2;
        const centerPoint = getPoint(centerX, centerY);
        const radius = getDistance(endPoint, centerPoint);
        ...
        function initTmap() {
          ...
          var polyline = new Tmapv2.Polyline({
            path: [
              new Tmapv2.LatLng(JsonObj[0].x, JsonObj[0].y),
              new Tmapv2.LatLng(JsonObj[9].x, JsonObj[9].y),
              new Tmapv2.LatLng(JsonObj[14].x, JsonObj[14].y),
            ],
